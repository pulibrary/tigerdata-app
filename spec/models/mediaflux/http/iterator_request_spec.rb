...
RSpec.describe Mediaflux::Http::IteratorRequest, type: :model do
  ...
  describe "action get-names" do
    let(:query_response) do
      filename = Rails.root.join("spec", "fixtures", "files", "iterator_response_get_names.xml")
      File.new(filename).read
    end
  ...
  describe "#action get-meta" do
    let(:query_response) do
      filename = Rails.root.join("spec", "fixtures", "files", "iterator_response_get_meta.xml")
      File.new(filename).read
    end
