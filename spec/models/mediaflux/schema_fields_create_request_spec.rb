...
RSpec.describe Mediaflux::SchemaFieldsCreateRequest, connect_to_mediaflux: true, type: :model do
  let(:mediaflux_url) { "http://0.0.0.0:8888/__mflux_svc__" }

  let(:mf_response_head) { "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<response>\n  <reply type=\"result\">\n    <result>\n      <tigerdata:project xmlns:tigerdata=\"tigerdata\"" }
  let(:mf_response_tail) { "changed=\"true\"/>\n    </result>\n  </reply>\n</response>\n" }
  ...
  describe "required indexed field" do
    it "submits the proper request" do
      ...
      request.resolve
      request.response_xml.to_s.include?(mf_response_head)
      request.response_xml.to_s.include?(mf_response_tail)
    end
  end
