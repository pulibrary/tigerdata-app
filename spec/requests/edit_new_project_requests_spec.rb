...
RSpec.describe "/edit_new_project_request", type: :request do
  describe "GET /edit" do
    it "redirects the client to the sign in path" do
      get edit_new_project_request_url(1)
    ...
    context "when the client is authenticated and is a regular user" do
      ...
      it "redirects them to the request path with a notice that they cannot modify the request" do
        sign_in user
        get edit_new_project_request_url(request.id)
        expect(response).to be_redirect
  ...
  describe "PATCH /update" do
    context "with valid parameters" do
      ...
      it "redirects the client to the sign in path" do
        put edit_new_project_request_url(1)
      ...
      context "when the client is authenticated and is a regular user" do
        ...
        it "redirects them to the request path with a notice that they cannot modify the request" do
          sign_in user
          put edit_new_project_request_url(request.id), params: { request: { project_title: "changed_title" } }
          expect(response).to be_redirect
          expect(response).to redirect_to(request_path(request.id))
          expect(flash.notice).to eq("The request has already been submitted.  You can no longer modify the request.")
        end
      ...
      context "when the client is authenticated and is a sysadmin" do
        ...
        let(:valid_request_params) do
          { request_title: "new title", project_title: "changed_title",
            ...
            description: "descr", project_purpose: "research", parent_folder: "parent",
            project_folder: "folder", project_id: "doi", quota: "200 GB", requested_by: "uid" }
        end
        ...
        it "updates the request and conveys the success message" do
          put edit_new_project_request_url(request.id), params: { request: valid_request_params }
          expect(response).to be_redirect
          expect(response).to redirect_to(request_path(request.id))
        ...
        context "with invalid parameters" do
          it "renders a response with 422 status (i.e. to display the 'edit' template)" do
            put edit_new_project_request_url(request.id), params: { request: { project_title: "" } }
