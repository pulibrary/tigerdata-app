 ...
 FactoryBot.create(:sponsor_and_data_manager, uid: "tigerdatatester", mediaflux_session: SystemUser.mediaflux_session) }
  ...
  context "When a project is created" do
    ...
    it "Sends project approval notifications" do
      expect {
 described_class.with(project_id:, approver: sponsor_and_data_manager_user).project_creation.deliver }.to change {
 ActionMailer::Base.deliveries.count }.by(1)
    ...
    context "when the project ID is invalid or nil" do
      ...
      it "does not enqueue an e-mail message and raises an error" do
        expect {
 described_class.with(project_id:).project_creation.deliver }.to raise_error(ArgumentError, 
"Invalid Project ID provided for the TigerdataMailer: #{project_id}")
      end
    end
  ...
  context "When a request is created" do
    it "Sends project creation requests" do
      expect {
 described_class.with(request_id:, submitter: sponsor_and_data_manager_user).request_creation.deliver }.to change {
 ActionMailer::Base.deliveries.count }.by(1)
      mail = ActionMailer::Base.deliveries.last
    ...
    context "when the request ID is invalid or nil" do
      ...
      it "does not enqueue an e-mail message and raises an error" do
        expect {
 described_class.with(request_id:, 
submitter: sponsor_and_data_manager_user).request_creation.deliver }.to raise_error(ArgumentError, 
"Invalid Request ID provided for the TigerdataMailer: #{request_id}")
      end
    end
  end
  ...
  context "when storage increase request is created" do
      ...
      let(:quota_breakdown) do
        { quota_used_human: "450 GB", project_files_human: "10 GB", old_versions_human: "20 GB", 
recycle_bin_human: "30 GB" }.with_indifferent_access
      end
    # let(:storage_increase_request) { FactoryBot.create(:storage_increase_request, project:) }
      ...
      it "Sends storage increase request notifications" do
        expect {
 described_class.with(project_id:, submitter: sponsor_and_data_manager_user, requested_capacity: requested_capacity, 
justification: justification, growth_expectation: growth_expectation, date_needed: date_needed, quota_breakdown: quota_breakdown).storage_increase_request.deliver }.to change {
   ActionMailer::Base.deliveries.count }.by(1)
        mail = ActionMailer::Base.deliveries.last
