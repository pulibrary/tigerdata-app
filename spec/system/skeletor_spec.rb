...
RSpec.describe "The Skeletor Epic", connect_to_mediaflux: true, js: true, integration: true do
  ...
  context "User Affiliated with Project" do
    let(:current_sysadmin) { FactoryBot.create(:sysadmin, uid: "sys123", mediaflux_session: SystemUser.mediaflux_session) }
    let(:current_user) { FactoryBot.create(:user, uid: "user4", mediaflux_session: SystemUser.mediaflux_session) }
    let(:datasponsor) { FactoryBot.create(:project_sponsor, uid: "ds123", mediaflux_session: SystemUser.mediaflux_session) }
    let(:datamanager) { FactoryBot.create(:data_manager) }
    it "allows the user affliated with the project to see their project" do
      # this is the feature flipper
      test_strategy = Flipflop::FeatureSet.current.test!
      test_strategy.switch!(:new_project_request_wizard, true)
      Affiliation.load_from_file(Rails.root.join("spec", "fixtures", "departments.csv"))
      ...
      click_on "Next"
      click_on "Approve request"
      page.find(:css, "#dropdownMenuButton").click
      click_on "Log out"
      sign_in datasponsor
      visit "/"
