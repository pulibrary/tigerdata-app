...
RSpec.describe TestAssetGenerator do
  ...
  describe "#generate" do
    ...
    context "multiple files" do
      ...
      it "creates test data" do
        allow(Mediaflux::Http::CreateAssetRequest).to receive(:new).with(session_token: "mediaflux_sessionid", pid: 1234, name: anything).and_return(test_collection_create)
        allow(Mediaflux::Http::TestAssetCreateRequest).to receive(:new).with(session_token: "mediaflux_sessionid", parent_id: "5678", count: 3).and_return(test_asset_create)
        subject.generate
        expect(user).to have_received(:mediaflux_session)
        expect(test_asset_create).to have_received(:resolve)
        expect(test_collection_create).to have_received(:id)
    ...
    context "Multiple levels" do
      ...
      let(:test_collection_create_level2) { instance_double(Mediaflux::Http::CreateAssetRequest, id: "2222") }
      ...
      it "creates collections and files" do
        allow(Mediaflux::Http::CreateAssetRequest).to receive(:new).with(session_token: "mediaflux_sessionid", pid: 1234, name: anything).and_return(test_collection_create)
        allow(Mediaflux::Http::CreateAssetRequest).to receive(:new).with(session_token: "mediaflux_sessionid", pid: "5678", name: anything).and_return(test_collection_create_level2)
        allow(Mediaflux::Http::TestAssetCreateRequest).to receive(:new).with(session_token: "mediaflux_sessionid", parent_id: "5678", count: 1).and_return(test_asset_create)
        allow(Mediaflux::Http::TestAssetCreateRequest).to receive(:new).with(session_token: "mediaflux_sessionid", parent_id: "2222", count: 1).and_return(test_asset_create_level2)
        subject.generate
        expect(test_asset_create).to have_received(:resolve)
        expect(test_asset_create_level2).to have_received(:resolve)
        expect(test_collection_create).to have_received(:id).twice
