...
function newTableRow(table, json) {
  ...
  const roCell = newCell(
    newRadio(`request[read_only_${json.id}]`, true, "true"),
  );
  newRow.appendChild(roCell);
  const rwCell = newCell(
    newRadio(`request[read_only_${json.id}]`, false, "false"),
  );
...
function moveNewUsers() {
  const originalCount = document.getElementsByName(
    "request[user_roles][]",
  ).length;
